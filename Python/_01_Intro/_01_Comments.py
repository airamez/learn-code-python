# A hash sign (#) defines a comment.

# The print command prints to the default output
print("Hello World")  # Prints "Hello world"

# Multiline comments are defined by ''' or """
'''
Multiple lines comment with single quotes
Line 1
Line 2
Line 3
Line 4
'''

"""
Multiple lines comment with double quotes
Line 1
Line 2
Line 3
Line 4
"""

print("Hello world with comments")

# TODO: This is a special type of comment to identify part of the program 
#       that needs attention to be completed/reviewed later
